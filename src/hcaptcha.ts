...
export interface hCaptcha {
  ...
  hcaptchaToken: (page: Page) => Promise<void>;
}

/**
 * @description Create form function to create request forms
 * @param sitekey website sitekey
 * @param host website host key
 * @param hl usually "en"
 * @param motionData json string of motion data
 * @param n optional
 * @param v optional
 * @param c optional
 * @returns
 ...
 */
const getAnswersTF = async (request_image: string, tasks: Tasks) => {
  let answers = new Map();
  const threads = [];
 ...
 */
const tryToSolve = async (userAgent: string, sitekey: string, host: string) => {
  // Create headers
  let headers = {
  ...
  let timestamp = Date.now() + rnd(30, 120);
  ...
  if (response.c === undefined) {
    form = createForm(sitekey, host, "en", new MotionData(timestamp, mm())).c;
  } else {
    form = createForm(
      sitekey,
      host,
      "en",
      new MotionData(timestamp, mm(), timestamp),
      response.c.type === "hsl"
 ...
 * @returns string response token
 */
const solveCaptcha = async (sitekey: string, host: string) => {
  try {
    while (true) {
 ...
 * @returns null
 */
export const hcaptcha = async (page: Page) => {
  ...
  await page.evaluate((token) => {
    (
      document.querySelector('[name="h-captcha-response"]') as HTMLInputElement
    ).value = token;
  }, token);
